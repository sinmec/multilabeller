...
def divide_image(window_size, image_rgb, stride_division=2):
    ...
    global_coord_reference = np.zeros((len(scan_I) * len(scan_J), 2), dtype=int)
    ...
    for i in scan_I:
        for j in scan_J:
            ...
            # Updating the global coord reference for furhter
            # image reconstruction
            global_coord_reference[index,0] = i_start
            global_coord_reference[index,1] = j_start

            index += 1
    return raw_img_subdivided, global_coord_reference
